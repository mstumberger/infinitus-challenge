...
class PozabljivImenik(Protocol):
            ...
            if cP.encode(data) == "".join(command):
                if 'command' in data:
                    ...
                    if parsed_command == ACTION.PUT:
                        ...
                        self.add_command(request_id, command_id, command, execution_time)
    ...
    def how_long_it_took(self, response, request_id, command_id):
        print("----\nRequest id:{},\ncommand id: {}\ncommand: {}\nresponse: {}"
              .format(request_id, command_id, self.REQUESTS[request_id]['commands'][command_id]["command"], response))
        ...
        if len(self.REQUESTS[request_id]) < 1:
